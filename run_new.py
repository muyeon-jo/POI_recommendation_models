    ...
    for e in range(args.epochs):
        ...
        if (e+1)%5 == 0:
            ...
            with torch.no_grad():
                ...
                end_time = int(time.time())
                print("eval time: {} sec".format(end_time-start_time))
def train_all_in_out(train_matrix, test_positive, val_positive, dataset):

    now = datetime.now()
    model_directory = "./model/"+now.strftime('%Y-%m-%d %H_%M_%S')+"all_in_out_sameItemvec"
    result_directory = "./result/"+now.strftime('%Y-%m-%d %H_%M_%S')+"all_in_out_sameItemvec"
    if not os.path.exists(model_directory):
        os.makedirs(model_directory)
    if not os.path.exists(result_directory):
        os.makedirs(result_directory)
