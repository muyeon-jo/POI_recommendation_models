...
class all_in_out(nn.Module):
    def __init__(self, item_num, embed_size, hidden_size, beta, region_embed_size): # embed_size : 64, beta : 0.5
        super(all_in_out, self).__init__()
        self.embed_size = embed_size # concat 연산 시 * 2
        self.item_num = item_num
        self.beta = beta
        self.hidden_size=hidden_size
        ...
        self.embed_history_outgoing = nn.Embedding(item_num, int(embed_size/4))

        self.embed_target_ingoing = nn.Embedding(item_num, int(embed_size/4))
        self.embed_target_outgoing = nn.Embedding(item_num, int(embed_size/4))
    ...
    def _init_weight_(self):
        # weight 초기화, 표준편차 : 0.01
        nn.init.normal_(self.embed_history_ingoing.weight, std=0.01)
        nn.init.normal_(self.embed_history_outgoing.weight, std=0.01)
        nn.init.normal_(self.embed_target_ingoing.weight, std=0.01)
        nn.init.normal_(self.embed_target_outgoing.weight, std=0.01)
        nn.init.normal_(self.embed_ingoing.weight, std=0.01)
        nn.init.normal_(self.embed_outgoing.weight, std=0.01)
    ...
    def attention_network(self, user_history, target_item, history_region_embed, target_region_embed):
        ...
        d: embedding size
        """
        a = self.embed_history_ingoing(user_history)
        b = self.embed_history_outgoing(user_history)
        history_ = torch.cat((a,b),dim=-1) # (b * n * d)
        history = torch.cat((history_, history_region_embed), -1)

        target_ = torch.cat((self.embed_history_outgoing(target_item),self.embed_history_ingoing(target_item)),-1) # (b * 1 * d)
        target = torch.cat((target_, target_region_embed),-1)
...
class nearPOI_embedding(nn.Module):
    def __init__(self, item_num, embed_size, hidden_size, beta, region_embed_size): # embed_size : 64, beta : 0.5
        super(nearPOI_embedding, self).__init__()
        self.embed_size = embed_size # concat 연산 시 * 2
        self.item_num = item_num
        self.beta = beta
        self.hidden_size=hidden_size
    ...
    def _init_weight_(self):
        ...
        nn.init.normal_(self.embed_target_ingoing.weight, std=0.01)
        nn.init.normal_(self.embed_target_outgoing.weight, std=0.01)
        nn.init.normal_(self.embed_near.weight, std=0.01)
        # nn.init.normal_(self.embed_outgoing.weight, std=0.01)
    ...
    def forward(self, history, target, near_pois, target_region):
        ...
        prediction = self.attention_network(history_tensor,target_tensor, region_emb[history], region_emb[target])
        return self.sigmoid(prediction)
    ...
    def self_attention(self, emb):
        q = emb[:,0,:].reshape(self.item_num,1,int(self.embed_size/2))
